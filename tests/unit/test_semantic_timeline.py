...
def _plan() -> NarrationPlan:
    ...
    ranges = (
        EvidenceRange(
            "range-approach", "situation-001", 1_000, 5_000, ("shot-001",),
            ("event-approach",), ("transcript-001",), ("frame-001",), "Jiro áp sát.",
            "event-approach", "APPROACH", "Jiro áp sát.", (uses[0],),
        ),
        EvidenceRange(
            "range-outcome", "situation-001", 6_000, 10_000, ("shot-002",),
            ("event-outcome",), ("transcript-002",), ("frame-002",), "Đối thủ gục.",
            "event-outcome", "OUTCOME", "Đối thủ gục.", (uses[1],),
        ),
    )
    cues = (
        NarrationCue(
            "cue-approach", "situation-001", "Jiro áp sát.", ("claim-001",),
            2_000, "ACTION", ("transcript-001",), ("frame-001",), ("shot-001",),
            ("Jiro",), ("Jiro",), (), (), 300, 300,
        ),
        NarrationCue(
            "cue-outcome", "situation-001", "Đối thủ gục.", ("claim-002",),
            7_000, "OUTCOME", ("transcript-002",), ("frame-002",), ("shot-002",),
            (), (), (), (), 300, 300,
        ),
    )
    unit = NarrationUnit(
        "unit-001", "situation-001", ("Jiro áp sát.", "Đối thủ gục."),
        "Jiro áp sát rồi hạ đối thủ.", "", "", ranges, "LOCKED", cues,
    )
    claims = (
...
def test_new_timeline_places_voice_after_visual_preroll() -> None:
    edl, timeline = build_semantic_timeline(
        _plan(), _tts(), source_duration_ms=12_000, policy=EditorialPolicy(
            target_minimum_ms=1_000, target_maximum_ms=60_000
        )
    )
    assert [segment.range_id for segment in edl.segments] == [
        "range-approach", "range-outcome"
    ]
